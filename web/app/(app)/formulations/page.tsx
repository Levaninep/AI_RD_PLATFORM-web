...
export default function FormulationsPage() {
  ...
  useEffect(() => {
    ...
    const requestedId = params.get("formulationId");
    setRequestedFormulationId(requestedId);
  }, []);
  ...
  useEffect(() => {
    ...
    if (editingId === requestedFormulationId) {
      return;
    }

    const target = formulations.find(
      (item) => item.id === requestedFormulationId,
    );
    if (!target) {
      return;
    }

    startEditFormulation(target);
  }, [requestedFormulationId, formulations, editingId]);
